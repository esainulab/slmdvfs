#!/bin/bash
# GPU Frequency Capping Experiment - SST-2, N=256, BERT-tiny (14M)
# Tests prajjwal1/bert-tiny at BASELINE + all 11 GPU frequencies
#
# Usage: ./run_gpu_freq_experiment_bert_tiny.sh
#
# Estimated time: ~3-4h total
...
FREQS=(BASELINE 1300 1224 1122 1020 918 816 714 612 510 408 306)
MODEL_NAME="prajjwal1/bert-tiny"
BATCH_SIZE=512
MODEL_LABEL="BERT-tiny"
...
OUTPUT_BASE="${SCRIPT_DIR}/runs/sst2_gpu_freq_bert_tiny"
...
preflight_checks() {
    ...
    if [ ! -f "$PARSER_SCRIPT" ]; then
        echo "Parser not found: $PARSER_SCRIPT (will skip parsing)"
    else
        echo "$PARSER_SCRIPT"
    fi
...
run_single() {
    ...
    local run_num=$2
    local total_runs=$3
    local start_time_global=$4

    print_header "RUN $run_num/$total_runs: ${MODEL_LABEL} @ ${freq}"

    local run_start=$(date +%s)
    local timestamp=$(date +%Y%m%d_%H%M%S)
    local experiment_name="${MODEL_LABEL}_SST2_${freq}"
    ...
    # Run training
    print_section "Training (${MODEL_LABEL}, QNLI, N=${MAX_LENGTH}, batch=${BATCH_SIZE})"
    set +e
...
run_experiments() {
    print_header "SST-2 GPU FREQUENCY EXPERIMENT — BERT-tiny (14M)"

    local total_runs=${#FREQS[@]}
    local current_run=0
...
SST-2 GPU Frequency Experiment — BERT-tiny (14M)
================================================
Date: $(date)
Model: $MODEL_NAME
Dataset: $DATASET
...
EOF
    ...
    for freq in "${FREQS[@]}"; do
        ...
        run_single "$freq" "$current_run" "$total_runs" "$start_time"
